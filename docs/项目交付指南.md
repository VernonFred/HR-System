...
**方案A: SQLite 部署（推荐小规模）**
- [ ] 空数据库文件（`hr.db`，首次启动自动创建）
- [ ] 数据库迁移脚本（alembic/versions/*.py）
- [ ] 数据库备份脚本（backup-sqlite.sh）
- [ ] 初始化管理员账号脚本

**方案B: PostgreSQL 部署（企业级）**
- [ ] 数据库创建 SQL（DBA执行）
- [ ] 数据库用户权限配置 SQL
- [ ] 数据库迁移脚本（alembic/versions/*.py）
- [ ] 数据库备份恢复说明
...
**说明**：
- ✅ SQLite：无需提供账号密码，数据库文件自动创建
- ✅ PostgreSQL：企业 DBA 自行创建数据库和用户，提供连接信息给部署人员
...
#### 1.4 AI 配置文件
- [ ] AI 提示词（`backend/app/core/ai/prompt_builder.py`）
- [ ] 岗位族配置（`backend/app/core/ai/job_families.json`）
- [ ] 简历解析提示词（`backend/app/api/resumes/parser.py`）
- [ ] 提示词管理文档（`docs/AI提示词管理文档.md`）

**说明**：
- ✅ AI 提示词全部编写在 Python 代码中，不是独立配置文件
- ✅ 修改提示词需要重启后端服务
- ✅ 提供了详细的提示词管理文档供运维人员参考
...
**数据库环境（二选一）**
- [ ] **方案A**: 无需安装（使用 SQLite，适合小规模）
- [ ] **方案B**: PostgreSQL 13+（企业级部署）

**数据库选型依据**：
| 场景 | 推荐方案 | 原因 |
|------|---------|------|
| 每日面试 < 10人 | SQLite | 零配置，开箱即用 |
| 每日面试 10-50人 | SQLite 或 PostgreSQL | 根据并发需求选择 |
| 每日面试 > 50人 | PostgreSQL | 高并发，支持分布式 |
| 多服务器部署 | PostgreSQL | SQLite 不支持多服务器 |
